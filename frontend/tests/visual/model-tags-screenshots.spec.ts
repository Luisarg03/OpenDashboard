...
test('model-tags visual capture + DOM probe (dark, default theme)', async ({ browser }) => {
  ...
  await shot(page, 'model-tags-dashboard.png');
  ...
  await page.waitForTimeout(500);
  await shot(page, 'model-tags-dashboard-scrolled.png');
  ...
  const tableOk = tableProbe !== null && tableProbe.kinds.includes('id');
  log(
  ...
  const costOk =
    costProbe !== null &&
    costProbe.kinds.includes('provider') &&
    costProbe.kinds.includes('id');
  log(
  ...
  await shot(spage, 'model-tags-session-detail.png');

  // DOM probe: header metadata row must contain an id pill.
  const headerProbe = await probePills(spage.locator('header'));
  const headerOk = headerProbe !== null && headerProbe.kinds.includes('id');
  log(
